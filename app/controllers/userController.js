...
const password=require('./../libs/sendMail')
...
let recoverPassword = (req, res) => {
    ...
    if (check.isEmpty(req.body.email)) {
        let apiResponse = response.generate(true, 'Email id is requied', 500, null)
        res.send(apiResponse)
    }
    else {
        userModel.findOne({ email: req.body.email }).exec((err, result) => {
            if (err) {
                console.log(err)
                logger.error(err.message, 'user Controller: recoverPassword', 10)
                let apiResponse = response.generate(true, `error occurred: ${err.message}`, 500, null)
                res.send(apiResponse)
            } else if (check.isEmpty(result)) {
                let apiResponse = response.generate(true, 'Invalid mail Id', 404, null)
                res.send(apiResponse)
            } else {
                let pass = shortid.generate();
                result.password = passwordLib.hashpassword(pass);
                ...
                let msg = `<h1 style="color:red">Hello ${result.firstName} ${result.lastName}</h1>
                             <p style="color:green">your new password is ${pass}</p>`;
...
}//end of recover passwo
module.exports = {
    logInFunction:logInFunction,
    socialLoginFunction:socialLogin,
    signUpFunction:signUpFunction,
    logOutFunction:logOutFunction,
    getAllusersFunction:getAllUsers,
    changePasswordFunction:changePassword,
    recoverPasswordFunction:recoverPassword
}